...
class HyperRIMModel(BaseModel):
    ...
    def gen_code(self, bs, w, h, levels=[], tensor_type=torch.randn):
        ...
        out_code = []
        for i in gen_levels:
            out_code.append(tensor_type(bs, self.code_nc * w * (2 ** i) * h * (2 ** i)).to(self.device))
        return out_code
    ...
    def compute_feature_loss(self, gen_feat, real_feat):
        # compute l2 feature loss given features
        result = 0
        # for i, g_feat in enumerate(gen_feat):
        cur_diff = torch.sqrt(torch.sum((gen_feat - real_feat) ** 2))
        result += cur_diff
        return result
    ...
    def get_features(self, level=-1):
        ...
        with torch.no_grad():
            gen_feat = self.netG(self.lr, self.code)
            # gen_feat, gen_shape = self.netF(gen_imgs[level])
            real_feat = self._get_target_at_level(level)

            out_dict['gen_feat'] = gen_feat
            out_dict['real_feat'] = real_feat
    ...
    def optimize_parameters(self, step, inter_supervision=False):
        ...
        if inter_supervision:
            ...
            outputs = self.netG(self.lr, self.code)
            l_g_total = 0.
            real_feat = self._get_target_at_level(-1)
            l_g_total += self.compute_feature_loss(outputs[0], real_feat)
        l_g_total = torch.sum(l_g_total)
        l_g_total = l_g_total.cpu()
        l_g_total.backward()
