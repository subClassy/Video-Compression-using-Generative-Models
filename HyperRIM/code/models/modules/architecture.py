...
class MLPNet(nn.Module):
    def __init__(self, in_nc, code_nc, out_nc):
        super(MLPNet, self).__init__()
        self.code_nc = code_nc
        MLP_dim = in_nc + code_nc
        self.tail = [
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(MLP_dim, MLP_dim*2, 1, 1, 0, bias=True),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(MLP_dim*2, MLP_dim*4, 1, 1, 0, bias=True),
            nn.LeakyReLU(negative_slope=0.2, inplace=True),
            nn.Conv2d(MLP_dim*4, out_nc, 1, 1, 0, bias=True)
        ]
    ...
    def forward(self, lr, codes):
        ...
        code = codes[0]
        bs, _, w, h = lr.shape
        x = torch.cat((lr, code.reshape(bs, self.code_nc, w, h)), dim=1)
        # x = torch.cat(lr, code, dim=1)
        outputs.append(self.tail(x))
        return outputs
...
class LPNet(nn.Module):
    def __init__(self, pnet_type='vgg', version='0.1'):
        ...
        model_path = os.path.abspath(
            os.path.join('.', 'code/models/weights/v%s/%s.pth' % (version, pnet_type)))
