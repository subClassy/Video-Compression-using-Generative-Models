...
class LRHRDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # img_HR = img_HR[:, 576 : 1536, :]
        
        # get LR image
        if self.paths_LR:
            LR_path = self.paths_LR[index]
            img_LR = util.read_img(self.LR_env, LR_path)
            # img_LR = img_LR[:, 576 : 1536, :]
        ...
        #     img_LR, img_HR = util.random_crop(img_HR, img_LR)

        # BGR to RGB, HWC to CHW, numpy to tensor
        # if img_HR.shape[2] == 3:
        #     img_HR = img_HR[:, :, [2, 1, 0]]
        #     img_LR = img_LR[:, :, [2, 1, 0]]
        img_HR = torch.from_numpy(np.ascontiguousarray(np.transpose(img_HR, (0, 2, 1)))).float()
        img_LR = torch.from_numpy(np.ascontiguousarray(np.transpose(img_LR, (0, 2, 1)))).float()
