...
src_path = '/media/himank/SSD/SelfC/results/test_codec_uvg_bf/selfc_mm_rocks_big_tv'
...
for seq_no, seq in enumerate(image_files):
    ...
    loss_h265 = total_variation_loss(np.abs(np.array(img_gt, dtype=np.float32) - np.array(img_h265, dtype=np.float32)))
    loss_selfc = total_variation_loss(np.abs(np.array(img_gt, dtype=np.float32) - np.array(img_selfc, dtype=np.float32)))
    
    losses_h265.append(loss_h265)
    losses_selfc.append(loss_selfc)
